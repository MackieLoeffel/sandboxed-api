...
void reset_random_s(void);
void new_sub_test(int);
...
int sqlite3_exec_s(
  sqlite3_sealed**, /* An open database */
  const char *sql /* SQL to be evaluated */
);

int sqlite3_prepare_v2_s(
  sqlite3_sealed **,            /* Database handle */
...
const unsigned char *sqlite3_column_text_s(
    sqlite3_stmt_sealed**,
    int iCol
);

int sqlite3_bind_int_s(sqlite3_stmt_sealed**, int, int);
int sqlite3_bind_int64_s(sqlite3_stmt_sealed**, int, sqlite3_int64);
int sqlite3_bind_text_s(sqlite3_stmt_sealed**,int,const char*,int);
...
static struct Global {
  sqlite3_sealed *db;               /* The open database connection */
  sqlite3_stmt_sealed *pStmt;       /* Current SQL statement */
...
void speedtest1_shrink_memory(void){
  ...
  /* if( g.bMemShrink ) sqlite3_db_release_memory(g.db); */
    if( g.bMemShrink ) fatal_error("shrink memory not supported");
#endif
}
...
void speedtest1_prepare(const char *zFormat, ...){
  ...
  if( g.bSqlOnly ){
    ...
    int rc;
    if( g.pStmt ) sqlite3_finalize_s(g.pStmt);
    rc = sqlite3_prepare_v2_s(&g.db, zSql, -1, &g.pStmt);
    if( rc ){
      /* fatal_error("SQL error: %s\n", sqlite3_errmsg(g.db)); */
      fatal_error("SQL error\n");
    }
  }
  sqlite3_free(zSql);
}
...
void speedtest1_run(void){
  ...
  assert( g.pStmt );
  g.nResult = 0;
  while( sqlite3_step_s(&g.pStmt)==SQLITE_ROW ){
    n = sqlite3_column_count_s(&g.pStmt);
    for(i=0; i<n; i++){
      const char *z = (const char*)sqlite3_column_text_s(&g.pStmt, i);
      if( z==0 ) z = "nil";
      len = (int)strlen(z);
  ...
  if( g.bReprepare ){
      fatal_error("reprepare not supported!");
    /* sqlite3_stmt *pNew; */
...
void testset_main(void){
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_int64_s(&g.pStmt, 1, (sqlite3_int64)x1);
    sqlite3_bind_int_s(&g.pStmt, 2, i);
    sqlite3_bind_text_s(&g.pStmt, 3, zNum, -1);
    speedtest1_run();
  }
  ...
  for(i=1; i<=n; i++){
    x1 = swizzle(i,maxb);
    speedtest1_numbername(x1, zNum, sizeof(zNum));
    sqlite3_bind_int_s(&g.pStmt, 1, i);
    sqlite3_bind_int64_s(&g.pStmt, 2, (sqlite3_int64)x1);
    sqlite3_bind_text_s(&g.pStmt, 3, zNum, -1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_int_s(&g.pStmt, 1, x1);
    sqlite3_bind_int_s(&g.pStmt, 2, x2);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_text_s(&g.pStmt, 1, zNum, len+1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_text_s(&g.pStmt, 1, zNum, len);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  sqlite3_bind_int_s(&g.pStmt, 1, est_square_root(g.szTest)*50);
  speedtest1_run();
  speedtest1_end_test();
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_int_s(&g.pStmt, 1, (sqlite3_int64)x1);
    sqlite3_bind_text_s(&g.pStmt, 2, zNum, -1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  for(i=1; i<=n; i++){
    ...
    speedtest1_numbername(x1, zNum, sizeof(zNum));
    sqlite3_bind_int_s(&g.pStmt, 2, i);
    sqlite3_bind_text_s(&g.pStmt, 1, zNum, -1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
...
void testset_cte(void){
  ...
  sqlite3_bind_text_s(&g.pStmt, 1, zPuz, -1);
  speedtest1_run();
  speedtest1_end_test();
  ...
  fatal_error("bind_double not supported");
...
void testset_fp(void){
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_text_s(&g.pStmt, 1, zFP1, -1);
    sqlite3_bind_text_s(&g.pStmt, 2, zFP2, -1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
...
void testset_rtree(int p1, int p2){
  ...
  for(i=1; i<=n; i++){
    ...
    sqlite3_bind_int_s(&g.pStmt, 1, i);
    sqlite3_bind_int_s(&g.pStmt, 2, x0);
    sqlite3_bind_int_s(&g.pStmt, 3, x1);
    sqlite3_bind_int_s(&g.pStmt, 4, y0);
    sqlite3_bind_int_s(&g.pStmt, 5, y1);
    sqlite3_bind_int_s(&g.pStmt, 6, z0);
    sqlite3_bind_int_s(&g.pStmt, 7, z1);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  iStep = mxCoord/n;
  for(i=0; i<n; i++){
    sqlite3_bind_int_s(&g.pStmt, 1, i*iStep);
    sqlite3_bind_int_s(&g.pStmt, 2, (i+1)*iStep);
    speedtest1_run();
  ...
  for(i=1; i<=n; i++){
    sqlite3_bind_int_s(&g.pStmt, 1, (i*251)%mxRowid + 1);
    speedtest1_run();
  }
  speedtest1_end_test();
  ...
  for(i=1; i<=n; i++){
    sqlite3_bind_int_s(&g.pStmt, 1, (i*257)%mxRowid + 1);
    speedtest1_run();
  }
  speedtest1_end_test();
...
void testset_orm(void){
  ...
  for(i=0; i<n; i++){
    ...
    len = (int)strlen(zNum);
    sqlite3_bind_int_s(&g.pStmt, 1, i^0xf);
    for(j=0; zType[j]; j++){
      switch( zType[j] ){
        case 'I':
        case 'T':
          sqlite3_bind_int64_s(&g.pStmt, j+2, x1);
          break;
        case 'F':
            fatal_error("bind_double not supported");
        ...
        case 'V':
        case 'B':
            fatal_error("bind_text64 not supported");
            (void) len;
...
void testset_trigger(void){
  ...
  for(jj=1; jj<=3; jj++){
    ...
    for(ii=0; ii<NROW2; ii+=3){
      sqlite3_bind_int_s(&g.pStmt, 1, ii*3);
      speedtest1_run();
    }
  }
  speedtest1_end_test();
  ...
  for(jj=0; jj<NROW2; jj++){
    sqlite3_bind_int_s(&g.pStmt, 1, jj*3);
    speedtest1_run();
  }
  speedtest1_end_test();
  ...
  for(jj=0; jj<NROW2; jj++){
    sqlite3_bind_int_s(&g.pStmt, 1, jj);
    speedtest1_run();
  }
  speedtest1_exec("COMMIT");
  speedtest1_end_test();
  ...
  */
  ...
  for(jj=1; jj<=NROW2*2; jj+=2){
    ...
    sqlite3_bind_int_s(&g.pStmt, 1, jj*2);
    sqlite3_bind_text_s(&g.pStmt, 2, zNum, -1);
    sqlite3_bind_int_s(&g.pStmt, 3, jj);
    speedtest1_run();
  }
  speedtest1_end_test();
...
int speedtest1_main(int argc, char **argv){
  ...
  if( sqlite3_open_s(zDbName, &g.db) ){
    fatal_error("Cannot open database file: %s\n", zDbName);
  }
#if SQLITE_VERSION_NUMBER>=3006001
  if( nLook>0 && szLook>0 ){
      fatal_error("Lookaside not supported!");
  ...
  if( doTrace ) fatal_error("trace not supported");
#endif
  if( mmapSize>0 ){
  ...
  if( showStats ){
    /* sqlite3_exec(g.db, "PRAGMA compile_options", xCompileOptions, 0, 0); */
      fatal_error("showStats not supported");
  }
